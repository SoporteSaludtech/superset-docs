# FCTAG04

**Tipo de Objeto:** Tabla Transaccional  
**Base de Datos:** `HVT_HOSVITAL`  
**Esquema:** `dbo`

---

## Estructura de Campos

| PK | Nombre de Columna | Tipo de Dato | Admite Nulos | Descripción / Rol Funcional |
| :-: | :--- | :--- | :-: | :--- |
| 🔑 | **`EMPCOD`** | `char(2)` | NO | Campo transaccional de Hosvital HIS |
| 🔑 | **`MCDpto`** | `char(9)` | NO | Campo transaccional de Hosvital HIS |
| 🔑 | **`MPNFac`** | `decimal(15,0)` | NO | Campo transaccional de Hosvital HIS |
| 🔑 | **`MATipDoc`** | `decimal(15,0)` | NO | Campo transaccional de Hosvital HIS |
| 🔑 | **`AFCnsAgr`** | `decimal(15,0)` | NO | Campo transaccional de Hosvital HIS |
|  | **`AFNFct`** | `decimal(15,0)` | SI | Campo transaccional de Hosvital HIS |
|  | **`AFNTip`** | `decimal(15,0)` | SI | Campo transaccional de Hosvital HIS |
|  | **`AFESTF`** | `smallint` | SI | Campo transaccional de Hosvital HIS |
